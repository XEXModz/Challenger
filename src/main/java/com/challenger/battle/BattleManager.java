...
public class BattleManager {
    ...
    public static void acceptRequest(ServerPlayer target, ServerPlayer challenger) {
        ...
        if (challenger.getServer() != null) {
            challenger.getServer().getPlayerList().getPlayers().forEach(p -> {
                if (!p.getUUID().equals(challengerUUID) && !p.getUUID().equals(targetUUID)) {
                    p.sendSystemMessage(Component.literal("§6[Battle] §e" + challenger.getName().getString() + " §6vs §e" + target.getName().getString() + " §6has begun! ⚔"));
                }
            });
        }
    }
    ...
    // --- Block ALL player vs player damage unless in a battle together ---
